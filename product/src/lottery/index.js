...
function setNextStatus(status = false) {
  isNexting = status;
}
    ...
    switch (target) {
      ...
      case "next":
        if (!isNexting) {
          ...
          saveData().then(res => {
            resetCard().then(res => {
              let c = basicData.luckyUsers[currentPrize.type]
              setPrizeData(currentPrizeIndex, c ? c.length : 0, true);
              currentLuckys = [];
              readyToLottery();
              setNextStatus(false);
            });
...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    selectCard();
    btns.lottery.innerHTML = "下一轮";
    btns.lottery.id = "next";
    //更新剩余抽奖数目的数据显示
    changePrize();
  });
}
